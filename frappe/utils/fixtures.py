...
def export_json_split_by_dt(doctype, folder_path, filters=None, or_filters=None):
	...
	# Remove stale per-DocType files (handles the case where all fields
	# for a DocType were removed since the last export)
	...
	for dt, names in sorted(grouped.items()):
		...
		file_path = os.path.join(folder_path, safe_dt + ".json")
